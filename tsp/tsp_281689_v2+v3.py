...
def main():
    ...
    while steady_state < STEADY_STATE:
        ...
        for i in range(len(new_generation)):
            if i < 15:
                offspring.append((new_generation[i], problem.evaluate_solution(np.array(new_generation[i]))))
            else:
                offspring = sorted([(i,j) for i,j in offspring], key= lambda t: t[1])
                solution_cost = problem.evaluate_solution(np.array(new_generation[i]))
                if offspring[14][1] > solution_cost:
                    offspring[14] = (new_generation[i],solution_cost)
        
        offspring = sorted([(i,j) for i,j in offspring], key= lambda t: t[1])
        
        best = offspring[0][1]
        best_sol = offspring[0][0]
        generation = np.array([i for i,j in offspring])
