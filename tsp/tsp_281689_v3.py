...
def tweak(solution: np.array, *, pm: float = .1) -> np.array:
    ...
    while p is None or p < pm:
        ...
        p = np.random.random()
    left = None
    while left is None or left == (solution.shape[0]-1):
        left = np.random.randint(0, solution.shape[0])
    right = solution[new_solution.index(left)+1]
'''
